      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-5 shrink-0 border-t border-gray-100 md:border-t-0 pt-5 md:pt-0">
          <div className="text-right hidden sm:block">
            <div className="font-[800] text-[18px] text-black bg-gray-50 px-3 py-1 rounded-xl shadow-inner border border-gray-100 inline-block mb-1">{booking.estimated_price}€</div>
          ...
          <div className="flex gap-2 w-full sm:w-auto">
            ...
            {!isPending && (
               <button onClick={onDetail} className="bg-white border border-gray-200 shadow-sm hover:shadow-md rounded-full transition-all font-bold px-5 py-3 text-[13px] text-black w-full sm:w-auto">Détails</button>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[2rem] p-8 max-w-lg w-full relative z-10 shadow-2xl animate-scale-in space-y-8">
        ...
        <div className="grid grid-cols-2 gap-4">
           <div className="bg-gray-50 rounded-2xl p-5 border border-gray-100 shadow-inner">
              <div className="text-[11px] font-[800] text-gray-400 uppercase tracking-wider mb-1">Téléphone</div>
              <div className="text-[15px] font-[800] text-black">{booking.client_phone}</div>
           </div>
           <div className="bg-gray-50 rounded-2xl p-5 border border-gray-100 shadow-inner">
              <div className="text-[11px] font-[800] text-gray-400 uppercase tracking-wider mb-1">Prix Estimé</div>
              <div className="text-[15px] font-[800] text-black">{booking.estimated_price}€</div>
           </div>
        </div>
        ...
        <div className="flex gap-3 pt-4">
           {booking.status === 'accepted' && (
              <button onClick={() => onAction(booking.id, "completed")} className="bg-black text-white hover:shadow-lg rounded-full transition-all font-bold px-6 py-4 flex-1">Marquer terminée</button>
           )}
           <a href={`tel:${booking.client_phone}`} className="bg-white border border-gray-200 shadow-sm hover:shadow-md rounded-full transition-all font-bold px-6 py-4 flex-1 text-center text-black">Appeler</a>
        </div>
      </div>
    </div>
  );
